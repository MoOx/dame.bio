...
let make = (~item: WPGraphQL.PostDetailFragment.t) => {
  ...
  <View style={styles["block"] /* accessibilityRole=`webAriaArticle */}>
      ...
      <View style={styles["metaRow"]}>
        <View style={styles["metaRowLeft"]}>
          ...
          <Text style={styles["date"]}>
            {(j`   ·   ` ++ item.dateGmt->Utils.frenchDate)->React.string}
          </Text>
        </View>
    ...
    <Comments databaseId=item.databaseId commentCounts=item.commentCount comments=item.comments />
    <SpacedView vertical=None>
      <TextSubtitle> {j`Dans le même style`->React.string} </TextSubtitle>
    </SpacedView>
    <PostList
      nodes={rootCategory
      ...
      ->Option.getWithDefault([])
      ->Array.keep(node =>
        node->Option.map(node => node.id != item.id)->Option.getWithDefault(false)
      )
