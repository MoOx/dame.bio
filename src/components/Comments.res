...
let styles = {
  ...
  StyleSheet.create({
    "comments": style(),
    "noComment": style(~textAlign=#center, ~fontSize=18., ~color="#d6d5d2", ~fontWeight=#_600, ()),
  })
}
...
let make = (
  ...
  <View style={styles["comments"]}>
    {switch comments->Option.flatMap(ts => ts.nodes)->Option.getWithDefault([]) {
    | [] => <>
        <Spacer size=L />
        <Text style={styles["noComment"]}>
          {j`Aucun commentaire pour l'instant. Laissez le vôtre !`->React.string}
        </Text>
        <Spacer size=L />
      </>
    | coms =>
