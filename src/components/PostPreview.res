...
let styles = {
  open Style
  StyleSheet.create({
    "wrapper": viewStyle(~flexGrow=1., ~flexShrink=1., ~flexBasis=340.->dp, ~overflow=#hidden, ()),
    "container": viewStyle(
      ~flexGrow=1.,
      ~flexShrink=1.,
    ...
    "titleText": textStyle(~fontSize=20., ~fontWeight=#_300, ~lineHeight=28., ~color="#1C1C1C", ()),
    "actionsWrapper": viewStyle(~position=#absolute, ~top=0.->dp, ~left=0.->dp, ~right=0.->dp, ()),
    "actions": viewStyle(
      ~flexDirection=#row,
      ~right=0.->dp,
      ~alignSelf=#flexEnd,
    ...
    "actionText": textStyle(~fontSize=10., ~color=ButtonLike.defaultColor, ()),
  })
}
...
let clearOptionalTimeout = optionalTimeoutRef =>
  optionalTimeoutRef.contents->Option.map(Js.Global.clearTimeout)->ignore

@react.component
let make = (~item: WPGraphQL.PostPreviewFragment.t, ~withWatercolorBottomRightCorner=false, ()) => {
  let id = item.id
  let rootCategory =
    item.categories
  ...
  let href =
    "/" ++
    (rootCategory->Option.flatMap(cat => cat.slug)->Option.getWithDefault("_")->Utils.encodeURI ++
  ...
  let uris =
    ...
    ->Array.keep(img => img.width->Option.getWithDefault("0")->Js.Float.fromString < 1024.)
  ...
  )[0]->Option.flatMap(i => i.sourceUrl)
  let categoryName =
    rootCategory
  ...
  let likes = switch item.likeCount->Option.getWithDefault(0) {
  | 0 => React.null
  | v => (v->string_of_int ++ "  ")->React.string
  }
  let comments = switch item.commentCount->Option.getWithDefault(0) {
  | 0 => React.null
  | v => ("  " ++ v->string_of_int)->React.string
  }
  <SpacedView key=id style={styles["wrapper"]} vertical=M horizontal=M>
    ...
    <SpacedView vertical=M horizontal=M style={styles["actionsWrapper"]} pointerEvents=#boxNone>
      <PlaceholderWithAspectRatio ratio=imageRatio />
      <SpacedView vertical=S horizontal=M style={styles["actions"]} pointerEvents=#boxNone>
