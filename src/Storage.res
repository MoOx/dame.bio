...
let freshStore = () =>
  store(~name="", ~email="", ~url="", ~likes=[], ~authToken=None, ~refreshToken=None)

let store = switch getIt("dame.bio") {
| Some(serializedStore) =>
  try Js.Json.parseExn(serializedStore)->Obj.magic catch {
  | exn =>
...
let saveStore = (): unit => saveIt("dame.bio", Js.Json.stringify(store->Obj.magic))
