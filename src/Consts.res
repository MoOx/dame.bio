...
let title = j`D'Âme Bio`
let titleTemplateHome = title ++ j` - %s`
let titleTemplate = j`%s - ` ++ title
let nickname = j`Aurélie`
...
Avec amour et bienveillance. Aurélie ♥︎`
...
À travers cet espace, je souhaite éveiller, transmettre, partager, se reconnecter à la nature, revenir à l’essentiel, impulser des prises de conscience et des changements positifs. `
...
let socialLinks = [
  {
    ...
    componentFunc: (~iconColor, ~iconSize) =>
      <SVGSocialInstagram
        fill=iconColor width={iconSize->Js.Float.toString} height={iconSize->Js.Float.toString}
      />,
  },
  {
    title: "Facebook",
